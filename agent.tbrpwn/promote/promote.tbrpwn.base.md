...
- Every request must triage first (Lite vs Havy) and then run either `LiteCycle` or `OuterCycle` as defined in `agent.tbrpwn.impl.md`.
...
- After each completed cycle, append `.tbrpwn/LOG.md` (`LiteCycle.vN` or `OuterCycle.vN`).
